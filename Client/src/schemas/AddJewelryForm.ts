...
export const jewelrySchema = z.object({
  ...
  sideStoneQuantity: z.number().positive({ message: "Stone quantity must be a positive number" }),
  stoneWeight: z.number().positive({ message: "Stone weight must be a positive number" }),
  goldType: z.string().min(1, { message: "Gold type is required" }),
  goldKarat: z.string().min(1, { message: "Gold karat is required" }),
  goldWeight: z.number().positive({ message: "Weight must be a positive number" }),
